...
struct S{ ll val; ll sum; ll min; ll max; int sz; };
struct M{ ll val; bool operator == (M x) { return val == x.val; } };
function<S(S)> formatting = [](S x) -> S { return {x.val, x.val, x.val, x.val, 1}; };
...
struct SplayTree{
    ...
    public:
        ...
        void insert(int k, ll x) { ROOT = insert(k,new SplayTree({x,x,x,x,1}),ROOT);}
        void push_front(ll x) { ROOT = insert(0,new SplayTree({x,x,x,x,1}),ROOT); }
        void push_back(ll x) { ROOT = insert((int)size(),new SplayTree({x,x,x,x,1}),ROOT);}
...
// verify: https://atcoder.jp/contests/code-festival-2014-exhibition-open/submissions/34134395 (カッコつけ)
...
// verify: https://judge.yosupo.jp/submission/100670
...
int main(){
    ...
    rep(i,2) st.push_back(0);
    rep(i,q){
        char c;
        int y,z;
        cin >> c >> y >> z;
        // rep(j,st.size()) cout << st.get(j) << " " ; cout << endl;
        if(c == '(') {
            int v = st.get(y-1) ;
            st.insert(y,v);
            st.apply(y,st.size(),{1});
        }
        if(c == ')') {
            int v = st.get(y-1) ;
            st.insert(y,v);
        ...
        if(c == 'D') {
            int v = st.get(y) - st.get(y-1) ;
            st.erase(y);
            if(v > 0) st.apply(y,st.size(),{-1});
            if(v < 0) st.apply(y,st.size(),{1});
        }
        if(c == 'Q') {
            int b = st.get(y-1);
